...
### Checkout Session status persistence v2 (prepared, unapplied)

The prepared `checkout_apply_card_event_v2` rollout preserves the existing v1 RPC while adding explicit `p_stripe_session_status` and `p_stripe_payment_status` arguments. The webhook route has no fallback to v1: it validates the signed Checkout Session status pair and passes both fields to v2 in the same transaction that changes authoritative attempt and order state. A card completion is accepted only as `complete` / `paid`; expiration requires `expired` and persists the signed Stripe payment status without creating authoritative paid state. Refund and dispute events cannot overwrite Session diagnostics.

The new migration is local and unapplied. Until it is separately reviewed and applied in every active environment, the application change must not be deployed because the updated webhook requires the v2 function. The success page remains read-only and continues to render authoritative order and attempt state rather than using the diagnostic Stripe status snapshot as payment authority.
