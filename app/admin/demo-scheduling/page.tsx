...
export default function DemoSchedulingAdmin() {
  ...
  function applyPayload(payload: Payload, reconcileSelection: boolean) {
    ...
    setData(normalized);
    setManagedAreaId((current) => normalized.serviceAreas.some((area) => area.id === current) ? current : normalized.serviceAreas[0]?.id ?? "");
  ...
  async function transition(action: string) {
    if (!selected) return;
    setMessage("");
    const response = await fetch(`/api/admin/demo-scheduling/requests/${selected.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, message: action === "deny" ? denial : null }) });
    const payload = await response.json().catch(() => ({}));
    if (response.ok) {
      ...
      setMessage(payload.warning ?? `Request ${action}d.`);
      await load();
  ...
  async function retryNotifications() {
    ...
    const payload = await response.json().catch(() => ({}));
    setMessage(response.ok ? (payload.retried ? "Failed notifications retried." : payload.message) : payload.error ?? "Retry failed.");
    await load();
  }
  ...
  const managedArea = data.serviceAreas.find((area) => area.id === managedAreaId) ?? null;
    ...
    <section className="mt-8 rounded-[2rem] bg-white p-5 shadow-sm sm:p-7">
      ...
      <div className="mt-7 grid gap-6 rounded-2xl border border-emerald-100 bg-emerald-50/50 p-5 lg:grid-cols-[minmax(0,1fr)_minmax(17rem,.75fr)]">
        <form onSubmit={saveDayPlan}>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="font-bold">Area / Region <span className="text-red-700">*</span><select required value={dayPlanDraft.regionId} onChange={(event) => setDayPlanDraft((current) => ({ ...current, regionId: event.target.value, cityId: "", customCity: "" }))} className="mt-2 w-full rounded-xl border bg-white p-3"><option value="">Choose an area</option>{availableAreas.map((area) => <option key={area.id} value={area.id}>{area.name}{area.active ? "" : " (Inactive)"}</option>)}</select></label><label className="font-bold">Specific City <span className="font-medium text-slate-500">(optional)</span><select value={dayPlanDraft.cityId} onChange={(event) => setDayPlanDraft((current) => ({ ...current, cityId: event.target.value, customCity: event.target.value ? "" : current.customCity }))} disabled={!dayPlanDraft.regionId} className="mt-2 w-full rounded-xl border bg-white p-3 disabled:bg-slate-100"><option value="">No specific city</option>{availableCities.map((city) => <option key={city.id} value={city.id}>{city.name}{city.stateAbbreviation ? `, ${city.stateAbbreviation}` : ""}{city.active ? "" : " (Inactive)"}</option>)}</select></label><label className="font-bold sm:col-span-2">Or enter a custom city <span className="font-medium text-slate-500">(optional)</span><input value={dayPlanDraft.customCity} maxLength={120} onChange={(event) => setDayPlanDraft((current) => ({ ...current, customCity: event.target.value, cityId: event.target.value ? "" : current.cityId }))} className="mt-2 w-full rounded-xl border bg-white p-3" /></label><label className="font-bold sm:col-span-2">Internal Note <span className="font-medium text-slate-500">(optional)</span><textarea rows={3} maxLength={500} value={dayPlanDraft.internalNote} onChange={(event) => setDayPlanDraft((current) => ({ ...current, internalNote: event.target.value }))} className="mt-2 w-full rounded-xl border bg-white p-3" /></label></div>
