...
export default function PricingAdminPage() {
  ...
  {message && <p role="status" className="mt-5 rounded-xl bg-white p-4 font-bold">{message}</p>}<p className="mt-6 font-bold">{filtered.length} pricing records</p><div className="mt-4 grid gap-4 lg:grid-cols-2">{filtered.map(item => <article key={`${item.kind}:${item.id}`} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.16em] text-emerald-700">{item.category}</p><h2 className="mt-1 text-xl font-black">{item.name}</h2><p className="text-sm text-slate-500">{[item.slug, item.brand, item.productName].filter(Boolean).join(" · ")}</p>{item.targetLabel && <p className="mt-1 text-sm font-bold">Target: {item.targetLabel}</p>}</div><button onClick={() => edit(item)} className="rounded-xl bg-slate-950 px-4 py-2 font-black text-white">Edit</button></div><div className="mt-4 flex flex-wrap gap-2"><span className="rounded-full bg-emerald-50 px-3 py-1 text-sm font-bold">Effective: {money(item.effectivePriceCents)}</span>{item.activeScheduleName && <span className="rounded-full bg-blue-100 px-3 py-1 text-sm font-black text-blue-900">Active schedule: {item.activeScheduleName}</span>}{item.publicStatus && <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-bold">{item.publicStatus}</span>}{item.quoteOnly && <span className="rounded-full bg-amber-100 px-3 py-1 text-sm font-black text-amber-900">Quote only — not self-service</span>}</div><PricingFacts item={item}/></article>)}</div></div>
