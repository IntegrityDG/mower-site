...
export default function ReferralAdminPage() {
  ...
  return <main className="min-h-screen bg-slate-100 p-5 text-slate-950 md:p-10"><div className="mx-auto max-w-7xl"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[.2em] text-emerald-700">IDS Admin</p><h1 className="text-4xl font-black">Referrals</h1></div><button onClick={async () => { await fetch("/api/admin/reviews/login", { method: "DELETE" }); setAuthed(false); }} className="rounded-xl border px-4 py-2 font-bold">Sign Out</button></div>
    ...
    <p className="mt-5 font-bold">{filtered.length} referrals</p><div className="mt-4 space-y-5">{filtered.map((row) => { const ready = isReadyForReview(row); const days = Math.max(1, Math.ceil((new Date(row.eligibleDate).getTime() - Date.now()) / 86400000)); return <article key={row.id} className={`rounded-[2rem] border bg-white p-6 shadow-sm ${ready ? "border-amber-400" : "border-slate-200"}`}><div className="flex flex-wrap justify-between gap-4"><div><h2 className="text-xl font-black">{row.referrerName}</h2><p className="text-sm text-slate-600">{row.referrerEmail} · {row.orderIdentifier}</p><p className="mt-1 text-sm font-bold">{row.brand} · {row.productName}</p></div><span className={`h-fit rounded-full px-3 py-1 text-sm font-black ${ready ? "bg-amber-100 text-amber-950" : "bg-slate-100"}`}>{ready ? "Ready for Review" : row.status}</span></div><div className="mt-5 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4"><p>Purchased: <b>{date(row.purchaseDate)}</b></p><p>{row.status === "pending" && !ready ? <><b>{days}</b> days remaining</> : <>Eligible: <b>{date(row.eligibleDate)}</b></>}</p><p>Order: <b>{row.orderStatus}</b></p><p>Payment: <b>{row.paymentStatus}</b></p><p>Base: <b>{money(row.baseRewardCents)}</b></p><p>Higher tier: <b>{money(row.higherTierRewardCents)}</b></p><p>Final reward: <b>{money(row.finalRewardCents)}</b></p><p>Tier: <b>{row.tierApplied ?? "Not determined"}</b></p></div>{row.disqualificationReason && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm"><b>Reason:</b> {row.disqualificationReason}</p>}<div className="mt-5 flex flex-wrap gap-2">{ready && <button onClick={() => void action(row, "qualify")} className="rounded-xl bg-emerald-600 px-4 py-2 font-black text-white">Mark Qualified</button>}{row.status === "qualified" && <button onClick={() => void action(row, "paid")} className="rounded-xl bg-emerald-600 px-4 py-2 font-black text-white">Mark Paid</button>}{["pending", "qualified"].includes(row.status) && <button onClick={() => void action(row, "disqualify")} className="rounded-xl bg-red-700 px-4 py-2 font-black text-white">Disqualify</button>}{row.status === "disqualified" && <button onClick={() => void action(row, "restore")} className="rounded-xl border px-4 py-2 font-black">Restore to Pending</button>}</div></article>; })}</div>
  </div></main>;
}
