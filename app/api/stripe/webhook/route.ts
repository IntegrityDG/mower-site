...
export async function POST(request: Request) {
  ...
  try {
    if (["checkout.session.completed","checkout.session.async_payment_succeeded","checkout.session.async_payment_failed","checkout.session.expired"].includes(event.type)) await handleSession(event.type,event.data.object as Stripe.Checkout.Session,event.id,expectedLivemode);
    else if (["payment_intent.requires_action","payment_intent.processing","payment_intent.partially_funded","payment_intent.succeeded","payment_intent.payment_failed","payment_intent.canceled"].includes(event.type)) await handleIntent(event.data.object as Stripe.PaymentIntent,event.id,expectedLivemode);
