...
import { applyCardEvent, findByPaymentIntentId, findBySessionId, finishWebhook, recordWebhookReceipt } from "@/lib/checkout/order-repository";
...
export async function POST(request: Request) {
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      const kind = reconcileCompletedSession({ livemode: session.livemode, mode: session.mode, paymentMethodTypes: session.payment_method_types, clientReferenceId: session.client_reference_id, metadata: session.metadata, currency: session.currency, amountTotal: session.amount_total, paymentStatus: session.payment_status }, record);
      ...
      await applyCardEvent({ p_kind: kind, p_session_id: session.id, p_payment_intent_id: paymentIntentId, p_order_id: record.orderId, p_attempt_id: record.attemptId, p_amount: session.amount_total, p_currency: session.currency, p_refunded: null, p_stripe_customer_id: stripeCustomerId, p_contact: { email: session.customer_details?.email, name: session.customer_details?.name, phone: session.customer_details?.phone, billingAddress: session.customer_details?.address, shippingAddress: session.collected_information?.shipping_details?.address } });
    } else if (event.type === "checkout.session.expired") {
      const session = event.data.object as Stripe.Checkout.Session;
      const record = await findBySessionId(session.id);
      if (!record) throw new Error("unknown");
      reconcileCompletedSession({ livemode: session.livemode, mode: session.mode, paymentMethodTypes: session.payment_method_types, clientReferenceId: session.client_reference_id, metadata: session.metadata, currency: session.currency, amountTotal: session.amount_total, paymentStatus: session.payment_status }, record);
      await applyCardEvent({ p_kind: "expired", p_session_id: session.id, p_payment_intent_id: objectId(session.payment_intent), p_order_id: record.orderId, p_attempt_id: record.attemptId, p_amount: record.totalCents, p_currency: record.currency });
      ...
      await applyCardEvent({ p_kind: event.type === "charge.refunded" ? "refund" : "dispute", p_session_id: record.sessionId, p_payment_intent_id: paymentIntentId, p_order_id: record.orderId, p_attempt_id: record.attemptId, p_amount: stripeObject.amount, p_currency: stripeObject.currency, p_refunded: event.type === "charge.refunded" ? (stripeObject as Stripe.Charge).amount_refunded : null });
    } else {
