...
export async function POST(request: Request, context: { params: Promise<{ guestId: string }> }) {
  ...
  catch (error) { const code=String((error as{message?:string})?.message??""); return Response.json({ error: /outside_direct_purchase_window/i.test(code) ? "The paid order is outside the direct 14-day Demo Party purchase window." : /guest_outside_qualifying_cap/i.test(code) ? "Only the first ten IDS-verified qualifying guests can receive a Demo Party direct-purchase referral." : /referral_guest_order_mismatch/i.test(code) ? "The paid order customer email must match the qualifying guest email." : /accessory/i.test(code) ? "Accessory-only orders do not qualify for the existing referral program." : "The guest and paid order could not be linked." }, { status: 409 }); }
}
