...
function DesktopHeader({ menuOpen, onOpen }: { menuOpen: boolean; onOpen: () => void }) {
  return <header className="border-b border-slate-300 bg-white"><div className="mx-auto flex max-w-7xl items-center gap-8 px-6 py-5 md:px-10"><div className="flex min-w-0 flex-1 items-center gap-8"><img src="/logo.png" alt="Integrity Distribution Systems" width={1250} height={500} className="h-auto w-[280px] shrink-0 object-contain lg:w-[340px]" /><div className="min-w-0 border-l border-slate-300 pl-8 text-left"><p className="text-3xl font-black leading-tight tracking-tight text-slate-950 lg:text-4xl">Integrity Distribution Systems</p><p className="mt-2 text-base font-bold uppercase tracking-[0.15em] text-emerald-700 lg:text-lg">Autonomous Lawn Care Solutions</p></div></div><button type="button" aria-label="Open desktop navigation" aria-expanded={menuOpen} aria-controls="desktop-navigation" onClick={onOpen} className="flex min-h-14 shrink-0 items-center gap-3 rounded-2xl border border-slate-300 bg-white px-5 font-black shadow-sm hover:bg-slate-50"><span className="flex flex-col gap-1.5" aria-hidden="true"><span className="h-0.5 w-7 bg-slate-900" /><span className="h-0.5 w-7 bg-slate-900" /><span className="h-0.5 w-7 bg-slate-900" /></span><span>MENU</span></button></div><div className="mx-auto max-w-7xl px-6 pb-4 md:px-10"><Link href="/remote-assistance" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-emerald-800 px-4 py-2 text-sm font-bold text-emerald-900">Remote Assistance</Link></div></header>;
}

function DesktopHero() {
  return (
    <section className="ids-purpose-background relative overflow-hidden px-10 py-16 text-white">
      <div className="ids-visual-copy-shadow relative z-10 mx-auto w-full max-w-5xl text-center">
        <div className="flex w-full justify-center">
          <img src="/images/cartoon-mowers.png" alt="Autonomous mower lineup" className="h-auto w-full max-w-[620px] object-contain drop-shadow-2xl" />
        </div>
        <p className="mt-4 w-full text-center text-2xl font-bold uppercase leading-tight tracking-[0.08em] text-emerald-300 md:text-[2rem]"><span className="block md:inline">A SMALL BUSINESS</span>{" "}<span className="block md:inline">WITH A SIMPLE</span>{" "}<span className="block md:inline">PURPOSE</span></p>
        <h1 className="mx-auto mt-5 max-w-4xl text-5xl font-black leading-[1.08] tracking-tight">Helping people get more time back for what matters most.</h1>
        <p className="mx-auto mt-7 max-w-4xl text-xl leading-8 text-slate-100">Integrity Distribution Systems is a small, Southeast Missouri&ndash;based business built around honesty, practical guidance, and doing right by the people we serve. We are not here to push the most expensive machine or chase the biggest sale. Our goal is to help each customer find a system that genuinely fits their property, needs, and budget.</p>
        <p className="mx-auto mt-5 max-w-4xl text-xl leading-8 text-slate-100">After spending a great deal of my own life working away from home, I understand how valuable time can be. Autonomous lawn care can reduce the hours and expense tied up in routine property maintenance, giving people more time with family, more room in their budget, and a little more opportunity to slow down and enjoy life.</p>
        <div className="mt-8 flex justify-center"><div className="inline-flex rounded-2xl border border-white/30 bg-slate-950/35 px-6 py-3 text-sm font-semibold text-white backdrop-blur-sm">Southeast Missouri Based &bull; Nationwide Equipment Sales &bull; Regional Hands-On Support</div></div>
      </div>
    </section>
  );
}
