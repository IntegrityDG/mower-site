    ...
    <div className="fixed inset-0 z-[100] md:hidden">
      ...
      <aside id="mobile-navigation" role="dialog" aria-modal="true" aria-label="Mobile navigation" className="absolute right-0 top-0 flex h-full w-[min(88vw,22rem)] flex-col bg-white p-5 shadow-2xl">
        ...
        <nav className="mt-5 flex flex-col gap-2" aria-label="Mobile homepage">
          ...
          {viewItems.slice(3).map((item) => <button key={item.view} type="button" aria-current={activeView === item.view ? "page" : undefined} onClick={() => choose(item.view)} className={`min-h-12 rounded-xl px-4 py-3 text-left text-sm font-black tracking-wide ${activeView === item.view ? "bg-emerald-50 text-emerald-800" : "text-slate-800 hover:bg-slate-100"}`}>{item.label}</button>)}
