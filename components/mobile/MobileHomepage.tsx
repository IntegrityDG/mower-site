...
export default function MobileHomepage() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-950 md:hidden">
      ...
      <main>
        {view === "home" && <><MobileHero /><HomeSalesSpecial /><section className="bg-white px-5 py-8 text-center"><button type="button" onClick={() => selectView("build")} className="w-full max-w-md rounded-2xl bg-emerald-600 px-6 py-5 text-lg font-black tracking-wide text-white shadow-lg transition hover:bg-emerald-700">BUILD YOUR SYSTEM</button><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-slate-600">Choose your equipment, customize your setup, and continue to secure checkout.</p></section><HomePriceMatch /><HomeBusinessSpotlight /></>}
        {view !== "home" && <section className="bg-slate-100 px-4 py-5"><button type="button" onClick={() => selectView("home")} className="mb-5 min-h-11 rounded-xl bg-white px-4 py-2 font-black text-emerald-700 shadow-sm">← Back to Home</button>{view === "build" && <div id="location-and-customer-path"><div className="mb-6 text-center"><p className="text-sm font-bold uppercase tracking-[0.25em] text-emerald-700">BUILD YOUR SYSTEM</p><h1 className="mt-2 text-3xl font-black">Choose your equipment and create the right setup for your property.</h1></div><NationwidePurchaseFlow /></div>}{view === "machines" && <div><div className="mb-6"><p className="text-sm font-black uppercase tracking-[0.2em] text-emerald-700">OUR MACHINES</p><h1 className="mt-2 text-3xl font-black">Featured Machines</h1><p className="mt-3 leading-7 text-slate-600">Explore our featured autonomous lawn care systems, then choose the machine you would like to see in action.</p><ScheduleDemoModal source="featured_machines" triggerClassName="mt-5 inline-flex min-h-14 w-full items-center justify-center rounded-2xl bg-emerald-600 px-6 py-4 font-black text-white shadow-md hover:bg-emerald-700" /></div><EquipmentCatalog /></div>}{view === "financing" && <HomeFinancing />}{view === "reviews" && <HomeReviews />}{view === "ids-action" && <IdsActionCarousel />}{view === "contact" && <HomepageContactSection />}</section>}
      </main>
