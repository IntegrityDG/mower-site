...
export default function AdminAppointmentOperations({ appointmentId, isParty }: { appointmentId: string; isParty: boolean }) {
  ...
  const bonusBenefit = detail.benefits.find((row) => row.benefit_type === "bonus_credit");
  ...
  return <section className="mt-6 border-t border-slate-200 pt-6">
    ...
    {isParty && detail.party && <>
      <div className="mt-5 grid gap-3 text-sm sm:grid-cols-3"><div className="rounded-xl bg-emerald-50 p-3"><p className="font-bold text-emerald-800">Base machine discount</p><p className="mt-1 font-black">{money(baseBenefit?.earned_cents)} earned · {money(baseBenefit?.consumed_cents)} consumed</p></div><div className="rounded-xl bg-emerald-50 p-3"><p className="font-bold text-emerald-800">Bonus credit</p><p className="mt-1 font-black">{money(bonusBenefit?.earned_cents)} earned · {money(bonusBenefit?.consumed_cents)} consumed</p><p className="mt-1 text-xs">Election: {String(bonusBenefit?.election ?? "Not elected").replaceAll("_", " ")}</p></div><div className="rounded-xl bg-emerald-50 p-3"><p className="font-bold text-emerald-800">Available now</p><p className="mt-1 font-black">{money(Number(baseBenefit?.earned_cents ?? 0) - Number(baseBenefit?.consumed_cents ?? 0))} base · {money(Number(bonusBenefit?.earned_cents ?? 0) - Number(bonusBenefit?.consumed_cents ?? 0))} bonus</p></div></div>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><h4 className="font-black">Registered guests ({detail.guests.length}) · benefits capped at 10 qualifying</h4><button disabled={busy} onClick={() => void toggleLock()} className="min-h-11 rounded-xl border px-4 font-bold">{detail.party.guest_list_locked ? "Unlock list" : "Lock list"}</button></div>
