...
import type { BonusCreditElection, DemoPartyGuest, DemoPartyPortal } from "@/lib/demo-party/types";
...
export default function HostPortal({ token, initial }: { token: string; initial: DemoPartyPortal }) {
  ...
  return <div className="space-y-7">
    ...
    {portal.demoFormat === "party" && <><section className="rounded-[2rem] bg-slate-950 p-6 text-white sm:p-8"><h2 className="text-2xl font-black">Verified benefits</h2><p className="mt-2 text-sm leading-6 text-slate-300">Only IDS-verified guests with at least one hour of attendance count. Benefits are capped at ten qualifying guests.</p><dl className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl bg-white/10 p-4"><dt className="text-sm text-slate-300">Qualifying guests</dt><dd className="mt-1 text-2xl font-black">{portal.benefits.qualifyingGuests}/10</dd></div><div className="rounded-2xl bg-white/10 p-4"><dt className="text-sm text-slate-300">Fee refund earned</dt><dd className="mt-1 text-2xl font-black">{money(portal.benefits.feeRefundCents)} / $100</dd></div><div className="rounded-2xl bg-white/10 p-4"><dt className="text-sm text-slate-300">Base machine discount</dt><dd className="mt-1 text-2xl font-black">{money(portal.benefits.baseMachineDiscountCents)} / $200</dd></div><div className="rounded-2xl bg-white/10 p-4"><dt className="text-sm text-slate-300">Bonus credit</dt><dd className="mt-1 text-2xl font-black">{money(portal.benefits.bonusCreditCents)} / $100</dd></div></dl>{portal.benefits.bonusCreditCents > 0 && <fieldset className="mt-6"><legend className="font-black">Apply the guest 6–10 bonus to one route</legend><div className="mt-3 flex flex-wrap gap-3"><button type="button" disabled={busy || portal.bonusCreditRedeemedCents > 0} aria-pressed={portal.bonusCreditElection === "accessories"} onClick={() => elect("accessories")} className={`min-h-11 rounded-xl border px-4 font-black ${portal.bonusCreditElection === "accessories" ? "border-emerald-400 bg-emerald-600" : "border-white/30"}`}>IDS-approved accessories</button><button type="button" disabled={busy || portal.bonusCreditRedeemedCents > 0} aria-pressed={portal.bonusCreditElection === "machine"} onClick={() => elect("machine")} className={`min-h-11 rounded-xl border px-4 font-black ${portal.bonusCreditElection === "machine" ? "border-emerald-400 bg-emerald-600" : "border-white/30"}`}>Same machine discount</button></div><p className="mt-3 text-sm text-slate-300">The election locks once any bonus is reserved or redeemed. Machine discounts do not stack with another discount route.</p></fieldset>}</section><section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/5 sm:p-8"><h2 className="text-2xl font-black">Demo Party program terms</h2><p className="mt-3 text-sm leading-7 text-slate-600">{DEMO_PARTY_DISCLAIMER}</p></section>

<section className="rounded-[2rem] bg-white p-6 shadow-xl shadow-slate-900/5 sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-black">Party guest list</h2><p className="mt-1 text-sm text-slate-600">Register legitimate invitees so IDS can plan attendance. Guest contact details remain private to IDS and this host portal; registration is not marketing consent. Benefits are capped at ten qualifying guests, but additional guests may attend.</p></div><span className="rounded-full bg-slate-100 px-4 py-2 font-black">{portal.guests.length} registered</span></div>{!paid && <p className="mt-5 rounded-xl bg-amber-50 p-4 font-bold text-amber-950">Guest management opens after payment is confirmed.</p>}{portal.guestListLocked && <p className="mt-5 rounded-xl bg-amber-50 p-4 font-bold text-amber-950">IDS has locked this guest list. Contact IDS to make a change.</p>}{canManageGuests && <form onSubmit={addGuest} className="mt-6 grid gap-3 sm:grid-cols-2"><label className="font-bold">Guest name<input name="fullName" required maxLength={160} autoComplete="name" className="mt-2 min-h-12 w-full rounded-xl border p-3" /></label><label className="font-bold">Guest email<input name="email" type="email" required maxLength={320} autoComplete="email" className="mt-2 min-h-12 w-full rounded-xl border p-3" /></label><label className="font-bold">Guest phone<input name="phone" type="tel" required maxLength={80} autoComplete="tel" className="mt-2 min-h-12 w-full rounded-xl border p-3" /></label><button disabled={busy} className="min-h-12 self-end rounded-xl bg-emerald-700 px-5 font-black text-white disabled:opacity-50">Add registered guest</button></form>}<ul className="mt-6 divide-y divide-slate-200">{portal.guests.map((guest) => <li key={guest.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-black">{guest.fullName}</p><p className="mt-1 text-sm text-slate-600">{guest.email} · {guest.phone}</p><p className="mt-1 text-xs font-bold uppercase tracking-wide text-slate-500">Attendance: {guest.qualificationStatus === "qualifying" ? "Verified qualifying" : guest.qualificationStatus === "not_qualifying" ? "Not qualifying" : "Pending IDS verification"}</p></div>{canManageGuests && guest.qualificationStatus === "pending" && <button type="button" disabled={busy} onClick={() => removeGuest(guest.id)} className="min-h-11 rounded-xl border border-red-200 px-4 font-bold text-red-700">Remove</button>}</li>)}</ul>{portal.guests.length === 0 && paid && <p className="mt-5 text-slate-500">No guests have been registered yet.</p>}</section></>}
