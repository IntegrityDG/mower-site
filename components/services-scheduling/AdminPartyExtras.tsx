...
export default function AdminPartyExtras({ appointmentId }: { appointmentId: string }) {
  ...
  return <section className="mt-8 grid gap-6 lg:grid-cols-2">
    <article className="rounded-[2rem] bg-white p-6 shadow-sm lg:col-span-2"><h2 className="text-xl font-black">Host screening &amp; party timing</h2><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4"><div><dt className="font-bold text-slate-500">Property relationship</dt><dd className="mt-1 font-black">{label(detail.party.property_relationship)}</dd></div><div><dt className="font-bold text-slate-500">Property type</dt><dd className="mt-1 font-black">{label(detail.party.property_type)}</dd></div><div><dt className="font-bold text-slate-500">Mowable acreage</dt><dd className="mt-1 font-black">{detail.party.mowable_acreage ?? "Not recorded"}</dd></div><div><dt className="font-bold text-slate-500">Purchase timeframe</dt><dd className="mt-1 font-black">{label(detail.party.purchase_timeframe)}</dd></div><div><dt className="font-bold text-slate-500">Equipment budget</dt><dd className="mt-1 font-black">{label(detail.party.equipment_budget)}</dd></div><div><dt className="font-bold text-slate-500">Actively considering</dt><dd className="mt-1 font-black">{detail.party.actively_considering_purchase ? "Yes" : "No"}</dd></div><div><dt className="font-bold text-slate-500">Guest arrival (Hour 3)</dt><dd className="mt-1 font-black">{arrival ? arrival.toLocaleString() : "Not scheduled"}</dd></div></dl><p className="mt-4 text-sm font-bold text-slate-600">Property authorization certification: {detail.party.property_authorization_certified ? "Confirmed" : "Not confirmed"}</p></article>
