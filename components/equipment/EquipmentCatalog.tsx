...
export function EquipmentCards({ products, aftermarketEnabled }: { products: CatalogProduct[]; aftermarketEnabled: boolean }) {
  ...
  return <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
    {machines.map((product) => { const quote = product.slug === "pandag-g1"; const available = product.isAvailable; return <article key={product.id} className="flex flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="flex flex-1 flex-col p-6"><p className="text-xs font-black uppercase tracking-[.16em] text-emerald-700">{product.brand}</p><h2 className="mt-3 text-2xl font-black">{product.name}</h2>{!available && <span className="mt-3 w-fit rounded-full bg-amber-100 px-3 py-1 text-sm font-black text-amber-950">Unavailable</span>}<p className="mt-3 flex-1 leading-7 text-slate-600">{product.homepageSummary ?? product.fullDescription}</p>
