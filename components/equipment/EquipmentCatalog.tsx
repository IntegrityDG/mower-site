...
export function EquipmentCards({ products, aftermarketEnabled }: { products: CatalogProduct[]; aftermarketEnabled: boolean }) {
  ...
  return <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
    {machines.map((product) => { const quote = product.slug === "pandag-g1"; const available = product.isAvailable; return <article key={product.id} className="flex flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="flex flex-1 flex-col p-6"><p className="text-xs font-black uppercase tracking-[.16em] text-emerald-700">{product.brand}</p><h2 className="mt-3 text-2xl font-black">{product.name}</h2>{!available && <span className="mt-3 w-fit rounded-full bg-amber-100 px-3 py-1 text-sm font-black text-amber-950">Unavailable</span>}<p className="mt-3 flex-1 leading-7 text-slate-600">{product.homepageSummary ?? product.fullDescription}</p>{product.slug === "yarbo" && product.variants.filter((variant) => variant.slug === "yarbo-y40p").map((variant) => <p key={variant.id} className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-bold text-amber-950">Choose the proven Y40 or next-generation Y40P Core. Y40P: {variant.publicStatus === "coming_soon" ? "Coming Soon" : variant.isAvailable ? "Available" : "Currently unavailable"}.</p>)}
        {!quote && available && <div className="mt-5"><p className="text-xs font-bold uppercase tracking-[.16em] text-slate-500">Starting at</p>{product.slug === "lymow-one-plus" ? <LymowPriceDisplay product={product} className="mt-1" priceClassName="text-2xl font-black text-emerald-700" /> : <YarboStartingPriceDisplay product={product} className="mt-1" priceClassName="text-2xl font-black text-emerald-700" />}</div>}
        {quote && available && <p className="mt-5 text-lg font-black text-emerald-700">Contact IDS for a Commercial Quote Today</p>}
        <div className="mt-5 grid gap-3"><Link href={`/equipment/${product.slug}`} className="rounded-xl bg-emerald-600 px-4 py-3 text-center font-black text-white hover:bg-emerald-700">{quote ? "View Pandag G1" : "View Details"}</Link>{quote && available && <Link href="/pandag/project-quote" className="rounded-xl border border-emerald-700 px-4 py-3 text-center font-black text-emerald-800">Request a Quote</Link>}</div>
      </div>
    </article>; })}
