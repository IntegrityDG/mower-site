...
type Field={name:string;label:string;kind?:"money"|"number"|"text"|"select";value?:string|number;options?:{value:string;label:string}[];optional?:boolean};
...
export default function InstallationAdminAction({installationId,action,title,fields=[],base={},disabled=false,reasonRequired=true,onSaved,scope="",endpoint}:Props){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      if(!body){
        ...
        for(const field of fields){const raw=String(form.get(field.name)??"");if(field.optional&&!raw)continue;let value:string|number=raw;
  ...
  return <form onSubmit={submit} className="space-y-3 rounded-xl border p-4"><h4 className="font-bold">{title}</h4>
    {pending?<><p className="text-sm">A saved operation is awaiting confirmation. Retrying keeps its original details.</p><details><summary>Saved details</summary><pre className="overflow-auto whitespace-pre-wrap break-all text-xs">{JSON.stringify(pending,null,2)}</pre></details></>:
      <>{fields.map(field=><label key={field.name} className="block text-sm">{field.label}{field.kind==="select"?<select name={field.name} required={!field.optional} defaultValue={field.value??""} className="mt-1 block w-full rounded border p-2"><option value="" disabled>Choose</option>{field.options?.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select>:<input name={field.name} required={!field.optional} type={field.kind==="number"?"number":"text"} inputMode={field.kind==="money"?"decimal":undefined} defaultValue={field.value} className="mt-1 block w-full rounded border p-2"/>}</label>)}<label className="block text-sm">Approval / reason{!reasonRequired&&" (optional)"}<textarea name="reason" required={reasonRequired} maxLength={2000} className="mt-1 block w-full rounded border p-2"/></label></>}
