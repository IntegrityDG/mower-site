...
export default function InstallationBookingForm() {
  ...
  useEffect(() => {
    try {const saved=sessionStorage.getItem("ids-installation-request");if(saved)setPending(JSON.parse(saved));} catch {setMessage("Saved request could not be read. Check your previous confirmation before starting another request.");}
  }, []);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      cashRequested: form.has("cashRequested"), idempotencyKey: crypto.randomUUID(),
    };
  ...
  async function send(body:Record<string,unknown>) {
    ...
    try {
      ...
      if (response.ok && typeof result?.token === "string" && /^[0-9a-f-]{36}$/i.test(result.token)) {
        ...
        if(["slot_unavailable","invalid_intake"].includes(result?.code)||response.status===400){sessionStorage.removeItem("ids-installation-request");setPending(null);retryAvailability();}
