...
create table public.installation_pricing_settings (
  ...
  included_one_way_travel_minutes integer not null default 120 check (included_one_way_travel_minutes >= 0),
  travel_hourly_cents integer not null default 3500 check (travel_hourly_cents >= 0),
...
create table public.installations (
  ...
  safety_status text not null default 'clear' check (safety_status in ('clear','suspended','remediation_pending','remediation_approved','terminated','weather_postponed')),
  safety_reschedule_used boolean not null default false,
  safety_notes text check (safety_notes is null or char_length(safety_notes) <= 5000),
  remediation_evidence_notes text check (remediation_evidence_notes is null or char_length(remediation_evidence_notes) <= 5000),
  ...
  draft_pricing jsonb,
  estimated_one_way_drive_minutes integer check (estimated_one_way_drive_minutes is null or estimated_one_way_drive_minutes >= 0),
  included_one_way_drive_minutes integer not null default 120 check (included_one_way_drive_minutes >= 0),
  excess_one_way_drive_minutes integer not null default 0 check (excess_one_way_drive_minutes >= 0),
  billable_travel_hours_per_direction integer not null default 0 check (billable_travel_hours_per_direction >= 0),
  total_billable_travel_hours integer not null default 0 check (total_billable_travel_hours >= 0),
  calculated_travel_charge_cents integer not null default 0 check (calculated_travel_charge_cents >= 0),
  approved_travel_charge_cents integer not null default 0 check (approved_travel_charge_cents >= 0),
  travel_manually_overridden boolean not null default false,
  travel_override_reason text check (travel_override_reason is null or char_length(travel_override_reason) <= 1000),
