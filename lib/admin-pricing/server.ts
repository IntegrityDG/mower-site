...
function rowToItem(kind: PricingKind, row: Record<string, unknown>, maps: { products: Map<string,string>; variants: Map<string,string>; options: Map<string,string>; packages: Map<string,string>; services: Map<string,string>; productServices: Map<string,string> }): PricingItem {
  ...
  return { id: String(row.id), kind, category: category[kind], name: String(names[kind] ?? "Unnamed"), slug: slugValue, brand: kind === "products" ? String(row.brand ?? "") || null : null, productName: product, publicStatus: typeof row.public_status === "string" ? row.public_status : null, quoteOnly: (kind === "products" && slugValue === "pandag-g1") || product?.toLowerCase().includes("pandag") === true, targetLabel, values, effectivePriceCents: effective(row) };
}
...
export async function readPricingCatalog(): Promise<PricingCatalog> {
  ...
  return { items: entries.flatMap(([kind, rows]) => rows.map((row) => rowToItem(kind, row, maps))) };
}
