...
function rowToItem(kind: PricingKind, row: Record<string, unknown>, maps: { products: Map<string,string>; variants: Map<string,string>; options: Map<string,string>; packages: Map<string,string>; services: Map<string,string>; productServices: Map<string,string> }, activeSchedule: ActivePriceSchedule | null, dealerCostCents: number | null): PricingItem {
...
export async function readPricingCatalog(): Promise<PricingCatalog> {
  ...
  const { data: privatePricing, error: privatePricingError } = await privateClient.from("catalog_internal_pricing").select("product_id,variant_id,option_id,package_id,service_id,product_service_id,dealer_cost_cents,updated_at");
  if (privatePricingError) throw privatePricingError;
  const costByTarget = new Map<string, number | null>();
  for (const row of (privatePricing ?? []) as Record<string, unknown>[]) {
    for (const [column, kind] of [["product_id","products"],["variant_id","variants"],["option_id","options"],["package_id","packages"],["service_id","services"],["product_service_id","product-services"]] as const) {
      if (row[column]) costByTarget.set(`${kind}:${row[column]}`, typeof row.dealer_cost_cents === "number" ? row.dealer_cost_cents : null);
    }
  }
  ...
  const now = Date.now();
  return { items: entries.flatMap(([kind, rows]) => rows.map((row) => { const target = targets[kind]; const activeSchedule = target ? selectActivePriceSchedule(schedules, target, String(row.id), now) : null; return rowToItem(kind, row, maps, activeSchedule, costByTarget.get(`${kind}:${row.id}`) ?? null); })) };
}
