import type { BonusCreditElection, DemoPartyBenefits } from "./types";

export const DEMO_FEE_CENTS = 10_000;
export const MAX_QUALIFYING_GUESTS = 10;
...
export function calculateDemoPartyBenefits(verifiedQualifyingGuests: number): DemoPartyBenefits {
  ...
  const feeRefundCents = Math.min(qualifyingGuests, 5) * GUEST_BENEFIT_INCREMENT_CENTS;
  const baseMachineDiscountCents = qualifyingGuests * GUEST_BENEFIT_INCREMENT_CENTS;
  const bonusCreditCents = Math.max(0, qualifyingGuests - 5) * GUEST_BENEFIT_INCREMENT_CENTS;
  return {
    qualifyingGuests,
    feeRefundCents,
    baseMachineDiscountCents,
    bonusCreditCents,
    maximumMachineDiscountCents: baseMachineDiscountCents + bonusCreditCents,
  };
}
...
export function chooseMachinePricingRoute(input: {
  ...
  bonusElection: BonusCreditElection | null;
}) {
  const earnedDiscount = input.baseMachineDiscountCents + (input.bonusElection === "machine" ? input.bonusCreditCents : 0);
  ...
  if (input.promotionalOrIdsPriceCents <= demoPartyPriceCents) {
    return { route: "existing_price" as const, priceCents: input.promotionalOrIdsPriceCents, consumedBaseCents: 0, consumedBonusCents: 0 };
  }
  ...
  return {
    ...
    consumedBaseCents,
    consumedBonusCents: Math.min(input.bonusElection === "machine" ? input.bonusCreditCents : 0, applicableDiscount - consumedBaseCents),
  };
}

export function accessoryCreditApplication(eligibleAccessoryAmountCents: number, availableBonusCreditCents: number) {
  if (!Number.isSafeInteger(eligibleAccessoryAmountCents) || eligibleAccessoryAmountCents < 0) throw new Error("Invalid eligible accessory amount.");
  if (!Number.isSafeInteger(availableBonusCreditCents) || availableBonusCreditCents < 0) throw new Error("Invalid Bonus Credit amount.");
  const appliedCents = Math.min(eligibleAccessoryAmountCents, availableBonusCreditCents);
  return { appliedCents, remainingMerchandiseCents: eligibleAccessoryAmountCents - appliedCents };
}
...
export function canChangeBonusCreditElection(consumedCents: number) {
  return consumedCents === 0;
}
