import { assertCanonicalProductionOrigin } from "@/lib/site-origin";
...
export function getStripeConfiguration(env: NodeJS.ProcessEnv = process.env): StripeConfiguration {
  ...
  try {
    assertCanonicalProductionOrigin(url, env, "APP_BASE_URL");
  } catch {
    throw new StripeConfigurationError("APP_BASE_URL must use the canonical IDS production origin.");
  }
