...
export function buildCardCheckoutSession(input: Input): Stripe.Checkout.SessionCreateParams {
  ...
  const lineTotal = lines.reduce((n,l) => n + (l.price_data.unit_amount ?? 0) * (l.quantity ?? 0), 0);
  if (input.serverDiscount) {
    if (!/^demo_party_[A-Za-z0-9_]+$/.test(input.serverDiscount.couponId) || input.serverDiscount.amountCents !== input.snapshot.discountCents || lineTotal - input.serverDiscount.amountCents !== input.snapshot.totalCents) throw new Error("Invalid server-authorized checkout discount.");
  } else if (lineTotal !== input.snapshot.totalCents) throw new Error("Stripe line total mismatch.");
