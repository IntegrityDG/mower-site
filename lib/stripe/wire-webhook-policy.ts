...
export function reconcileWireSession(session: Session, record: RecordIdentity) {
...
export function reconcileWireIntent(intent: Intent, record: RecordIdentity) {
  if (intent.livemode || intent.paymentMethodTypes.length!==1 || intent.paymentMethodTypes[0]!=="customer_balance" || intent.metadata.order_id!==record.orderId || intent.metadata.attempt_id!==record.attemptId || intent.metadata.payment_method!=="wire_transfer" || intent.currency!==record.currency || intent.amount!==record.totalCents || !Number.isSafeInteger(intent.amountReceived) || intent.amountReceived<0) throw new WebhookReconciliationError("Wire PaymentIntent reconciliation failed.");
