...
import { additionalLabor, balanceDueAt, cancellationDepositRefund, initialAmount, locationRefusalLaborRefund, travelQuote, undergroundLabor, DEFAULT_PRICING, type PricingSnapshot } from "./policy";

export const ADMIN_ACTOR = "IDS shared administrator";
export type WorkSession = { id: string; status: string; duration_minutes: number | null; duration_seconds?: number | null; corrected_from_id: string | null; started_at: string; ended_at: string | null; notes: string | null };
...
export const cumulativeWorkMinutes = (sessions: WorkSession[]) => activeWorkSessions(sessions).reduce((n,s) => n + Math.round(workSessionMinutes(s)*60*1e6),0)/60e6;
...
export function approvedPricing(value: unknown): PricingSnapshot {
  ...
  if (Object.keys(p).length!==Object.keys(DEFAULT_PRICING).length || Object.keys(p).some(k => !(k in DEFAULT_PRICING))) throw new Error("invalid_pricing");
  ...
  if(p.includedLaborMinutes!==240 || p.laborIncrementMinutes!==15 || p.undergroundSegmentFeet!==10 || p.includedOneWayTravelMinutes!==120
    || p.depositCents>initialAmount(p)) throw new Error("locked_installation_terms");
  return {...p};
}
export function requiredFundsBeforeDeadline(state: AdminState) {
  ...
  return net>=initialAmount(i.pricing_snapshot)+i.approved_travel_charge_cents;
}
const fields: Record<string,string[]>={
  ...
  safety:["status","evidence"],session_start:[],session_stop:["status"],session_correct:["sessionId","durationMinutes"],
...
export function prepareAdminOperation(state: AdminState, input: unknown, defaults: PricingSnapshot, now = new Date()) {
  const body=validateAdminInput(input), i=state.installation, action=body.action, reason=body.reason;
  const pricing=approvedPricing(i.pricing_snapshot??i.draft_pricing??defaults);
  ...
  const running=state.sessions.find(s=>s.status==="running"), minutes=cumulativeWorkMinutes(state.sessions);
  ...
  const reconcile=(kind:string,target:number,description:string)=>{
    ...
    add(kind==="materials"?"material":kind==="labor"?"additional_labor":kind==="underground"?"underground_labor":"travel_time",target-previous,description,kind);
  };
  const newSlot=()=>{
  ...
  if(action==="approve") {
    ...
    Object.assign(patch,{pricing_snapshot:pricing,approved_at:now.toISOString(),status:"deposit_due",balance_due_at:balanceDueAt(i.requested_start_at).toISOString(),
      deposit_due_cents:Date.parse(i.requested_start_at)-now.getTime()<=72*3600000?initialAmount(pricing)+i.approved_travel_charge_cents:pricing.depositCents});
    reconcile("travel",i.approved_travel_charge_cents,"Approved travel charge");
  } else if(action==="pricing") {
    if(running)throw new Error("pause_work_before_pricing_change");
    const next=approvedPricing(body.pricing);patch.draft_pricing=next;
    if(i.pricing_snapshot){patch.pricing_snapshot=next;patch.deposit_due_cents=Date.parse(i.requested_start_at)-Date.parse(i.approved_at)<=72*3600000?initialAmount(next)+i.approved_travel_charge_cents:next.depositCents;}
  } else if(action==="travel") {
    const q=travelQuote(integer(body.oneWayMinutes,"travel_minutes"),pricing,body.approvedChargeCents===undefined?undefined:integer(body.approvedChargeCents,"travel"));
    Object.assign(patch,{estimated_one_way_drive_minutes:q.estimatedOneWayMinutes,included_one_way_drive_minutes:q.includedOneWayMinutes,excess_one_way_drive_minutes:q.excessOneWayMinutes,
      ...
      approved_travel_charge_cents:q.approvedChargeCents,travel_manually_overridden:q.manuallyOverridden,travel_override_reason:q.manuallyOverridden?reason:null});
    if(i.pricing_snapshot){
      reconcile("travel",q.approvedChargeCents,`Approved travel: ${reason}`);
      if(Date.parse(i.requested_start_at)-Date.parse(i.approved_at)<=72*3600000)patch.deposit_due_cents=initialAmount(pricing)+q.approvedChargeCents;
    }
  } else if(action==="cash") {
    ...
    requireApproved();requireOpen();if(i.cash_status!=="approved"||running||minutes>0||i.special_cash_failure_reschedule)throw new Error("cash_failure_not_available");
    ...
    requireApproved();if(!i.special_cash_failure_reschedule||!i.reschedule_opportunity_used||!i.balance_due_at||now.getTime()<Date.parse(i.balance_due_at)||requiredFundsBeforeDeadline(state))throw new Error("deposit_forfeiture_not_due");
    if(i.deposit_forfeited_cents>0)throw new Error("deposit_already_forfeited");
    add("credit",pricing.depositCents-pricing.laborCents,`Cash-failure deposit forfeiture; unused labor credited, materials separately reconciled: ${reason}`,"cash_failure_forfeit");
    ...
    if(!requiredFundsBeforeDeadline(state)&&(i.special_cash_failure_reschedule||(!inside72&&i.cash_status!=="approved"&&!i.payment_arrangement_reason)))throw new Error("payment_deadline_missed_requires_ids_review");
    ...
    patch.status="in_progress";sessions.push({status:"running",notes:reason||null});
    ...
    sessions.push({status:"corrected",corrected_from_id:id,duration_minutes:integer(body.durationMinutes,"work_minutes"),notes:reason});
    ...
    requireApproved();reconcile("materials",integer(body.actualCents,"materials")-pricing.materialsAllowanceCents,`Actual eligible materials reconciled: ${reason}`);
  } else if(action==="labor") {
    requireApproved();if(running)throw new Error("pause_work_before_labor_reconciliation");reconcile("labor",Math.round(additionalLabor(minutes,pricing)),`Additional labor for ${minutes} cumulative work minutes`);
  } else if(action==="underground") {
    requireApproved();if(typeof body.feet!=="number"||!Number.isFinite(body.feet)||body.feet<0||body.feet>10000)throw new Error("invalid_underground_feet");
    ...
    requireApproved();requireOpen();if(running)throw new Error("pause_work_before_settlement");
    ...
    requireOpen();if(running||minutes>0)throw new Error("use_work_or_safety_termination");
    if(action==="decline"&&i.status!=="requested")throw new Error("invalid_transition");
    if(i.pricing_snapshot){const retained=pricing.depositCents-cancellationDepositRefund(pricing.depositCents,i.requested_start_at,now);add("credit",retained-pricing.laborCents,`Cancellation labor settlement; materials separately reconciled: ${reason}`,"cancellation");}
    patch.status=action==="decline"?"declined":"cancelled";
  }
  const afterPricing=(patch.pricing_snapshot??i.pricing_snapshot??patch.draft_pricing??i.draft_pricing??defaults) as PricingSnapshot;
