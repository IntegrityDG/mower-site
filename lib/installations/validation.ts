export type InstallationIntake={name:string;email:string;phone:string;address:string;equipment:string|null;preferredLocation:string|null;internetAvailability:"yes"|"no"|"unsure";undergroundRequested:boolean;estimatedUndergroundFeet:number;startAt:string;groundingAcknowledged:boolean;responsibilitiesAcknowledged:boolean;termsAcknowledged:boolean;undergroundAcknowledged:boolean;cashRequested:boolean;idempotencyKey:string};
export function validateInstallationIntake(raw:unknown):{ok:true;value:InstallationIntake}|{ok:false;errors:Record<string,string>}{
  ...
  const allowed=["name","email","phone","address","equipment","preferredLocation","internetAvailability","undergroundRequested","estimatedUndergroundFeet","startAt","groundingAcknowledged","responsibilitiesAcknowledged","termsAcknowledged","undergroundAcknowledged","cashRequested","idempotencyKey"];
  ...
  for(const k of ["groundingAcknowledged","responsibilitiesAcknowledged","termsAcknowledged"])if(x[k]!==true)errors[k]="Acknowledgement is required.";
  for(const k of ["undergroundRequested","undergroundAcknowledged","cashRequested"])if(x[k]!==undefined&&typeof x[k]!=="boolean")errors[k]="Choose a valid acknowledgement.";
  ...
  return Object.keys(errors).length?{ok:false,errors}:{ok:true,value:{name,email,phone,address,startAt,idempotencyKey:idempotencyKey.toLowerCase(),equipment,preferredLocation,
    ...
    groundingAcknowledged:true,responsibilitiesAcknowledged:true,termsAcknowledged:true,undergroundAcknowledged:x.undergroundAcknowledged===true,cashRequested:x.cashRequested===true}};
}
