...
export async function notifyNewDemoRequest(request: DemoRequest) {
  ...
  try {
    result.ids = await deliver(request, "ids_new_request", () => sendIdsNotification({ subject: `New ${request.demoFormat === "party" ? "Demo Party" : "Private Demo"} Request`, text: `A new request is pending IDS approval.\n\n${details(request)}\n\nAdmin: /admin/demo-scheduling` }));
  } catch { result.ids = "failed"; }
  try {
    result.customer = await deliver(request, "customer_request_received", () => sendServerEmail({
...
export async function notifyDemoMoreInformation(request: DemoRequest) {
  return deliver(request, "customer_more_information", () => sendServerEmail({
    to: request.customerEmail,
...
export async function notifyDemoPaymentConfirmed(request: DemoRequest, token: string | null) {
  ...
  const calendarEmail = process.env.DEMO_CALENDAR_EMAIL ?? process.env.NOTIFY_EMAIL;
  try {
    if (!calendarEmail) throw new Error("DEMO_CALENDAR_EMAIL and NOTIFY_EMAIL are missing.");
    result.calendar = await deliver(request, "ids_calendar_invite", () => sendServerEmail({ to: calendarEmail, subject: `IDS ${request.demoFormat === "party" ? "Demo Party" : "Private Demo"} — ${request.customerName}`, text: details(request), attachments: [attachment(request, calendarEmail, "IDS Demo Calendar")] }));
  } catch { result.calendar = "failed"; }
  return result;
}

export async function notifyDemoDenial(request: DemoRequest) {
  return deliver(request, "customer_denied", () => sendServerEmail({ to: request.customerEmail, subject: "Update on Your IDS Demo Request", text: `Hello ${request.customerName},\n\nMachine requested: ${request.equipmentInterest ?? "Not specified"}\n\n${request.adminMessage}\n\nYour requested appointment was not confirmed. You may return to the IDS website to request another available time.` }));
}
