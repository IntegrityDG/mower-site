...
import { DEMO_APPOINTMENT_BUFFER_MINUTES } from "@/lib/scheduling/config";
import { addDays, centralLocalToUtc } from "./time";
import { excludeInstallationConflicts } from "@/lib/scheduling/availability";
...
export async function getAvailableSlots(start:string,end:string,now=new Date()){const c=getSupabaseServiceClient();const rangeStart=centralLocalToUtc(start,"00:00")?.toISOString(),rangeEnd=centralLocalToUtc(addDays(end,1),"00:00")?.toISOString();if(!rangeStart||!rangeEnd)return[];const bufferedRangeStart=new Date(Date.parse(rangeStart)-DEMO_APPOINTMENT_BUFFER_MINUTES*60_000).toISOString(),bufferedRangeEnd=new Date(Date.parse(rangeEnd)+DEMO_APPOINTMENT_BUFFER_MINUTES*60_000).toISOString();const [{data:rules,error:rErr},{data:exceptions,error:eErr},{data:requests,error:qErr},{data:settings,error:sErr},{data:typeSettings,error:tErr},{data:installations,error:iErr},{data:fieldService,error:serviceError}]=await Promise.all([c.from("demo_availability_rules").select("id,weekday,enabled,start_time,end_time"),c.from("demo_availability_exceptions").select("id,starts_at,ends_at,all_day,reason").lt("starts_at",rangeEnd).gt("ends_at",rangeStart),c.from("demo_requests").select("requested_start_at,requested_end_at,status,appointment_type").in("status",["pending","approved"]).lt("requested_start_at",bufferedRangeEnd).gt("requested_end_at",bufferedRangeStart),c.from("demo_settings").select("scheduling_horizon_days").eq("id",true).single(),c.from("appointment_type_settings").select("duration_minutes,public_active").eq("appointment_type","demo").single(),c.from("installations").select("requested_start_at,requested_end_at,status").lt("requested_start_at",rangeEnd).gt("requested_end_at",rangeStart),c.rpc("ids_service_field_occupancy",{p_start:rangeStart,p_end:rangeEnd})]);if(serviceError&&!["PGRST202","42883"].includes(serviceError.code))throw serviceError;if(iErr&&!["PGRST205","42P01"].includes(iErr.code))throw iErr;if(rErr||eErr||qErr||sErr||tErr)throw rErr??eErr??qErr??sErr??tErr;if(!typeSettings.public_active)return[];return excludeInstallationConflicts(generateAvailableSlots({start,end,now,rules:rules??[],exceptions:exceptions??[],requests:requests??[],duration:Number(typeSettings.duration_minutes),horizon:Number(settings.scheduling_horizon_days)}),[...(installations??[]),...(fieldService??[])]);}
