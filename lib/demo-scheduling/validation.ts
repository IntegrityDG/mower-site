import { DEMO_SOURCES, type DemoSource } from "./types";
...
export function validateDemoRequest(input:unknown){
 ...
 const name=clean(b.name,160),email=clean(b.email,320).toLowerCase(),phone=clean(b.phone,80),address=clean(b.propertyAddress,500),startAt=clean(b.requestedStartAt,40),source=clean(b.source,40)as DemoSource,equipmentInterest=clean(b.equipmentInterest,80)||null,idempotencyKey=clean(b.idempotencyKey,36),honeypot=clean(b.company,100);
 ...
 const expectedEquipment=source==="featured_lymow"?"Lymow One Plus":source==="featured_yarbo"?"Yarbo Core":null;
 if(equipmentInterest!==expectedEquipment)errors.equipmentInterest="Equipment interest does not match the request source.";
