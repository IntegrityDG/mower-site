...
export function priceFromRow(row: PublicPriceRow, now = Date.now()): CatalogPrice {
  ...
  return { displayMsrpPriceCents:null, regularPriceCents:row.regular_price_cents, salePriceCents:row.sale_price_cents, currentPriceCents:saleIsActive ? row.sale_price_cents : row.regular_price_cents, showPublicPrice:row.show_public_price, contactForPricing:row.contact_for_pricing, promotionLabel:saleIsActive ? row.promotion_label : null, saleIsActive };
}
