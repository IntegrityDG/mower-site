...
export type CheckoutOptionRow = PriceableRow & { product_id: string; option_slug: string; name: string; description: string | null; minimum_quantity: number; maximum_quantity: number | null };
...
const YARBO_HIDDEN = new Set(["yarbo-plow-module", "yarbo-tow-hitch"]);
...
export function validateCheckoutEligibility(request: CheckoutRequest, catalog: CheckoutCatalog) {
  ...
  if (selectedOptions.some(({ option }) => !YARBO_MODULES.has(option.option_slug))) reject("INCOMPATIBLE_SELECTION", "Only approved Yarbo modules may be selected.");
  if (request.selection.purchaseMode === "complete-system") {
    ...
    if (request.selection.includeBaseProduct || selectedOptions.length) reject("YARBO_PACKAGE_DOUBLE_COUNT", "A Yarbo package cannot include standalone Core or module selections.");
    ...
    return { variant: null, selectedPackage, selectedOptions: [], packageItems: items, moduleOnlyWarning: null };
  }
  ...
  if (!request.selection.includeBaseProduct && !selectedOptions.length) reject("MISSING_CONFIGURATION", "Choose Yarbo Core or at least one module.");
  return { variant: null, selectedPackage: null, selectedOptions, moduleOnlyWarning: !request.selection.includeBaseProduct && selectedOptions.length ? "Yarbo Core is not included. These modules require an existing Yarbo Core to operate." : null };
}
