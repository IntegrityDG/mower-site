...
export function checkoutAttemptIdempotencyKey(requestId: string) { return createHash("sha256").update([CHECKOUT_POLICY_VERSION, PAYMENT_SECURITY_POLICY_ID, PAYMENT_SECURITY_POLICY_VERSION, "card", requestId.toLowerCase()].join(":" )).digest("hex"); }
