import { optionalServicesSelected } from "./optional-services";
...
export function canonicalCheckoutRequest(input: CheckoutRequest) {
  return { ...(optionalServicesSelected(input.optionalServices) ? { optionalServices: input.optionalServices } : {}), requestId: input.requestId.toLowerCase(), paymentMethod: input.paymentMethod, selection: { ...input.selection, options: [...input.selection.options].sort((a,b) => a.optionId.localeCompare(b.optionId)) }, customer: { name: clean(input.customer.name), email: clean(input.customer.email)?.toLowerCase() ?? null, phone: clean(input.customer.phone) }, referral: input.referral ? { referrerName: clean(input.referral.referrerName), referrerEmail: clean(input.referral.referrerEmail)?.toLowerCase() } : null, shippingAddress: { ...input.shippingAddress, line1: clean(input.shippingAddress.line1), line2: clean(input.shippingAddress.line2), city: clean(input.shippingAddress.city), state: clean(input.shippingAddress.state)?.toUpperCase(), postalCode: clean(input.shippingAddress.postalCode) } };
}
