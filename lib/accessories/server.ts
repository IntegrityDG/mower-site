...
import { AFTERMARKET_DISCLAIMER, type AccessoryCatalogResponse, type AccessoryItem, type AccessorySettings } from "./types";
...
export async function readAccessoryCatalog(admin = false): Promise<AccessoryCatalogResponse> {
  ...
  if (!admin) items = items.filter((item) => item.visible && (item.tab !== "aftermarket" || settings.aftermarketEnabled)).map((item) => ({...item, visible: undefined, publicStatus: undefined}));
...
export async function setAccessoryRemoved(id: string, removed: boolean) {
  const { data, error } = await getSupabaseServiceClient().from("catalog_options").update({ public_status: removed ? "hidden" : "active", accessory_listing_enabled: !removed, show_in_builder: removed ? false : undefined, updated_at: new Date().toISOString() }).eq("id", id).eq("admin_managed", true).not("accessory_tab", "is", null).select("*").single();
