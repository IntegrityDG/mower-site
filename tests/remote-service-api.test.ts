...
runInNewContext(ts.transpileModule(readFileSync(new URL("../lib/service/api.ts", import.meta.url), "utf8"), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText, {
  ...
  require: (name: string) => name === "server-only" ? {} : name === "./validation" ? { ServiceError } : name === "@/lib/dealer-network/api" ? { dealerNetworkOrigin: () => "https://example.invalid" } : assert.fail(`Unexpected dependency ${name}`),
});
const api = exports as typeof import("../lib/service/api");
const request = (body = "{}", origin: string | null = "https://example.invalid", extra: Record<string, string> = {}) => new Request("http://localhost:3058/api/service/cases", { method: "POST", headers: { "Content-Type": "application/json", ...(origin ? { Origin: origin } : {}), ...extra }, body });
test("Service mutation accepts only its configured public origin even when Next normalizes the internal hostname", async () => {
  ...
  for (const origin of [null, "https://attacker.invalid", "http://localhost:3058", "null"]) await assert.rejects(api.serviceBody(request("{}", origin)), error => error instanceof ServiceError && error.status === 403);
});
...
test("Service request parsing rejects malformed JSON, wrong media type and bounded oversized streams", async () => {
  ...
  await assert.rejects(api.serviceBody(request("{}", "https://example.invalid", { "Content-Type": "text/plain" })), /JSON is required/);
  await assert.rejects(api.serviceBody(request("{}", "https://example.invalid", { "Content-Length": String(256 * 1024 + 1) })), error => error instanceof ServiceError && error.status === 413);
